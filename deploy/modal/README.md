...
The script verifies that an unsupported domain is rejected before delivery, that an allowed-domain registration is accepted, and that Resend reports the resulting confirmation message as `delivered`, `opened`, or `clicked`. Confirmation retains its existing link behavior and may be completed from the recipient inbox. Password-reset email is different: this API-native service sends a reset token plus `PUT /users/password` JSON/curl instructions, not a browser reset link; use that API flow privately for final password-reset acceptance.
