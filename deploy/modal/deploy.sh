...
config/credentials/production.yml.enc must exist and be committed to Git.
config/credentials/production.key must never be committed.
...
[[ -s "$REPO_ROOT/$PRODUCTION_CREDENTIALS" ]] || die "missing/empty production credentials: $PRODUCTION_CREDENTIALS"
git -C "$REPO_ROOT" ls-files --error-unmatch "$PRODUCTION_CREDENTIALS" >/dev/null 2>&1 || \
  die "production credentials must be committed before deployment: $PRODUCTION_CREDENTIALS"

if git -C "$REPO_ROOT" ls-files --error-unmatch "$PRODUCTION_KEY" >/dev/null 2>&1; then
  die "production credentials key must never be committed: $PRODUCTION_KEY"
fi

if [[ -n "$(git -C "$REPO_ROOT" status --porcelain --untracked-files=all -- deploy/modal "$PRODUCTION_CREDENTIALS")" ]]; then
  die 'Modal deployment files or production credentials have uncommitted changes; commit or clean them before deployment'
fi
