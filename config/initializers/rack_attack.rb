...
class Rack::Attack
  ...
  # Per-user: 60 requests per 60 s per JWT sub (authenticated endpoints).
  # Keys on the JWT `sub` claim when present, falls back to IP for unauthenticated.
  throttle("req/user", limit: 60, period: 60) do |req|
    if req.env["devise.jwt.claims"]
      req.env["devise.jwt.claims"]["sub"]
    end
  end
