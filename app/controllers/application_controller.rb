...
class ApplicationController < ActionController::API
  ...
  def route_not_found
    logger.warn("Route not found: #{request.request_method} #{request.fullpath} (IP: #{request.remote_ip})")
    render json: { error: I18n.translate("errors.route_not_found") }, status: :not_found
  end
