...
- JWT-based sign in, sign out, and token refresh with Refresh Token Rotation (RTR) and token revocation via denylist.
...
- Automated JWT denylist and refresh token cleanup via Active Job (Solid Queue) and Rake task.
...
| **3.2.2** | Minimum supported version |
