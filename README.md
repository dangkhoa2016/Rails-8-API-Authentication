...
- **Browser clients** use the signed **HttpOnly, Secure, SameSite=Lax `refresh_token`
  cookie**. Do not put the raw token in `localStorage` or `sessionStorage`.
- **Native / mobile / CLI clients** read the raw token from the JSON body
  (`refresh_token`) and send it back via the `refresh_token` param or the
...
[`docs/JWT_LIFECYCLE.md`](./docs/JWT_LIFECYCLE.md) for the full policy.
...
/users/password` sends an email containing the raw reset token, the canonical
...
- [docs/DEPLOYMENT.vi.md](./docs/DEPLOYMENT.vi.md) - Vietnamese deployment guide
- [deploy/modal/README.md](./deploy/modal/README.md) and [Vietnamese guide](./deploy/modal/README.vi.md) - provider-neutral Modal recipe (`https://<your-modal-url>`)
- [deploy/beam/README.md](./deploy/beam/README.md) and [deploy/huggingface/README.md](./deploy/huggingface/README.md) - additional verified deployment recipes
- [docs/RELEASE_PROCESS.md](./docs/RELEASE_PROCESS.md) and [Vietnamese guide](./docs/RELEASE_PROCESS.vi.md) - release process
- [docs/releases/v1.0.0-acceptance.md](./docs/releases/v1.0.0-acceptance.md) - v1.0.0 acceptance record
...
They are derived from a Flaticon sticker design. See [public/license.md](./public/license.md) for the source, attribution, and license details.
