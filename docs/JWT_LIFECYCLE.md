...
The current token duration is **900 seconds (15 minutes)**. This can be modified in `config/initializers/devise.rb` via `jwt.expiration_time`.
...
| Invalid Token (bad format / decode error) | 401 | error body only | — | — |
...
Malformed JWTs do not produce `token_info`; `ApplicationController` rescues `JWT::DecodeError` and returns `401 Unauthorized` (`{ "error": "Invalid token" }`).

---

## Refresh Tokens

The access JWT only lives for **15 minutes**. To avoid forcing the user to re-enter their credentials, the system also issues a long-lived **refresh token** with a **7-day** validity.

### How a refresh token is issued

On `POST /users/sign_in`, `Users::SessionsController#create` generates a new refresh token and:

1. Stores only its **SHA-256 digest** in the `refresh_tokens` table (the raw token is never persisted).
2. Returns it to the client in two ways:
   - As an **HttpOnly, Secure, SameSite=Lax cookie** named `refresh_token` (protects web clients against XSS).
   - In the JSON body under the `refresh_token` key (for mobile / non-cookie clients).

### Transport security policy

The two transports have distinct audiences and rules:

- **Browser clients** must use the signed **HttpOnly, Secure, SameSite=Lax `refresh_token` cookie**. The raw token must **not** be stored in `localStorage` or `sessionStorage`, which any script running on the page can read.
- **Native / mobile / CLI clients** that cannot manage cookies receive the raw token in the JSON body (`refresh_token`) and send it back via the `refresh_token` param or the `X-Refresh-Token` header.

Rules that apply to every client:

- Send refresh-token requests over **TLS in production** — the token is a bearer credential and must never travel in plaintext.
- Never persist raw tokens in application logs, analytics, or crash reports; only the SHA-256 digest is stored server-side.
- Store raw tokens in secure platform storage on non-cookie clients; treat them like passwords.

Removing the refresh token from the JSON body to make the API **cookie-only** is a breaking change for non-cookie clients and will ship as a separate release.

### The `refresh_tokens` table

| Column | Purpose |
|---|---|
| `token_digest` | SHA-256 of the raw token (unique) |
| `family_id` | Groups rotated tokens belonging to the same login session |
| `expires_at` | Expiration (`7.days.from_now`) |
| `revoked_at` | Set when the token is rotated, the user signs out, or reuse is detected |
| `user_agent` / `ip_address` | Context captured at issuance time |

### Refresh Token Rotation

`POST /users/tokens/refresh` accepts a refresh token (via `refresh_token` cookie, `refresh_token` param, or `X-Refresh-Token` header) and:

1. Looks up the token by digest.
2. If the token was already **revoked** → **reuse detected**: the whole `family_id` group is revoked and a `401 Security alert` is returned (protects against stolen-token replay).
3. If expired or the account is inactive → the token is revoked and `401` is returned.
4. Otherwise the old token is **rotated**: it is revoked and a new token with the **same `family_id`** is issued, alongside a fresh access JWT.

### Sign out

`DELETE /users/sign_out` now also revokes the active refresh token (from cookie, param, or `X-Refresh-Token` header) before clearing the cookie.

### Cleanup

`CleanExpiredRefreshTokensJob` runs every hour in production (see `config/recurring.yml`) and deletes tokens whose `expires_at` or `revoked_at` is older than 7 days.
...
| `app/models/refresh_token.rb` | Model for refresh tokens — digest, rotation, reuse detection, family revocation |
| `app/controllers/users/tokens_controller.rb` | `POST /users/tokens/refresh` — rotates the refresh token and issues a new access JWT |
| `app/jobs/clean_expired_refresh_tokens_job.rb` | Deletes expired/revoked refresh tokens older than 7 days |
| `config/recurring.yml` | Production recurring schedule for `CleanExpiredJwtDenylistsJob` and `CleanExpiredRefreshTokensJob` |
