...
| `/user/profile` | GET | ❌ 422 + token info, or 401 invalid-token error | ✅ (Self) | ✅ |
| `/user/me`, `/user/whoami` | GET | ❌ 422 + token info, or 401 invalid-token error | ✅ (Self) | ✅ |
...
| `/users/:id` | GET | ❌ 401 | ✅ if `id` matches (403 if mismatch) | ✅ Any |
| `/users/:id` | PUT | ❌ 401 | ✅ if `id` matches (403 if mismatch) | ✅ Any |
| `/users/:id` | DELETE | ❌ 401 | ✅ if `id` matches (403 if mismatch) | ✅ Any |
...
- Calling `GET /user/profile` with a malformed token returns `401 { "error": "Invalid token" }` instead of a `token_info` payload.
...
Request → Warden / Devise resolves current_user from JWT
        → authorize_user_access
              ...
              └─ otherwise             → 403 "You must be an administrator..."
```
...
Malformed JWTs take a different path: `ApplicationController` rescues the decode failure and returns `401 { "error": "Invalid token" }`.
