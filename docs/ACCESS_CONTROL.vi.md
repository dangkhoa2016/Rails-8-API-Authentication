...
| `/user/profile` | GET | ❌ 422 + token info hoặc 401 invalid-token error | ✅ (bản thân) | ✅ |
| `/user/me`, `/user/whoami` | GET | ❌ 422 + token info hoặc 401 invalid-token error | ✅ (bản thân) | ✅ |
...
| `/users/:id` | GET | ❌ 401 | ✅ nếu `id` khớp (403 nếu không khớp) | ✅ bất kỳ |
| `/users/:id` | PUT | ❌ 401 | ✅ nếu `id` khớp (403 nếu không khớp) | ✅ bất kỳ |
| `/users/:id` | DELETE | ❌ 401 | ✅ nếu `id` khớp (403 nếu không khớp) | ✅ bất kỳ |
...
- Gọi `GET /user/profile` với token bị lỗi format sẽ nhận `401 { "error": "Invalid token" }`, không có `token_info`.
...
Request → Warden / Devise xác định current_user từ JWT
        → authorize_user_access
              ...
              └─ còn lại               → 403 "You must be an administrator..."
```
...
JWT bị malformed đi theo nhánh khác: `ApplicationController` rescue lỗi decode và trả `401 { "error": "Invalid token" }`.
