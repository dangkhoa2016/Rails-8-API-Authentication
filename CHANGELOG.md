...
- API-native password reset: email supplies a reset token and `PUT
  /users/password` instructions; no browser reset form is bundled. The
  compatibility `GET /users/password/edit` route prints no-store API
  instructions without mutating the token.
